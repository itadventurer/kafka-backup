...
gen_messages() {
    ...
    for NUM in {$START_NUM..$(($START_NUM+$COUNT-1))} ; do
        ...
        echo $(gen_message $PARTITION $NUM $SIZE)
    done
    echo ""
}
...
verify_messages() {
    ...
    while read MESSAGE ; do
        ...
        if [ "$KEY_CHECKSUM" != "$VALUE_CHECKSUM" ] ; then
            echo "Partition $KEY_PARTITION, Key $KEY_NUM, KChk $KEY_CHECKSUM, vlength $(echo "$VALUE" | wc -c), vchk: $VALUE_CHECKSUM"
...
kafka_connect_load_connector() {
    ...
    confluent load $NAME -d /tmp/connect.properties
